...
class TicketAccessTests(TestCase):
    ...
    def test_ramais_page_displays_filter_options(self):
        ...
        self.assertContains(response, '<th>Colaborador</th>', html=True)
        self.assertContains(response, '<th>Departamento</th>', html=True)

    def test_ramais_page_orders_rows_by_collaborator_name(self):
        PhoneExtension.objects.create(
    ...
    def test_ti_can_export_phone_extensions_pdf_puts_blank_collaborator_last(self):
        PhoneExtension.objects.create(
            department='Sala Reuniao Grande',
            name='',
            phone='(16) 3353-8428',
            extension='8428',
            email='',
    ...
    def test_ti_can_create_contrato_when_custom_field_row_is_partial(self):
        self.client.login(username='usuario.ti', password='senha@123')
        response = self.client.post(
            reverse('chamados_contratos'),
            data={
                'name': 'Contrato com linha parcial',
                'notes': 'Cadastro padrao.',
                'amount': '300.00',
                'contract_start': '2026-01-01',
                'contract_end': '2026-12-31',
                ...
                'contract_custom_fields_new-0-label': '',
                'contract_custom_fields_new-0-field_type': ContractCustomField.FieldType.TEXT,
                'contract_custom_fields_new-0-value_text': 'Algo incompleto',
                'contract_custom_fields_new-0-value_number': '',
                'contract_custom_fields_new-0-value_bool': '',
            },
        )
        ...
        self.assertRedirects(response, reverse('chamados_contratos'))
        contrato = ContractEntry.objects.get(name='Contrato com linha parcial')
        self.assertEqual(contrato.custom_fields.count(), 0)
